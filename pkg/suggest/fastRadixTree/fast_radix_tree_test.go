...
//		res := Match("mum", "mum")
//		assert.Equal(t, res, CommonPrefix{commonPrefix: "mum", queryNameRemain: "", nodeValueRemain: ""})
//	})
//
//	t.Run("mum, mumum", func(t *testing.T) {
//		res := Match("mum", "mumum")
//
//		assert.Equal(t, res, CommonPrefix{"mum", "", "um"})
//	})
//
//	t.Run("mum, mumum", func(t *testing.T) {
//		res := Match("mum", "mumum")
...
//		res := Match("mum", "apple")
//
//		assert.Equal(t, res, CommonPrefix{"", "mum", "apple"})
//	})
//
//	t.Run("empty query", func(t *testing.T) {
//		res := Match((""), "mum")
//
//		assert.Equal(t, res, CommonPrefix{"", "", "mum"})
//	})
//}
...
func TestNode_Add(t *testing.T) {
	t.Run("empty", func(t *testing.T) {
		node := NewNode()
		q := prefixTree.NewQuery("a", 100)
		node.Add(q)
		res := node.Search("")
		require.Equal(t, []*prefixTree.QueryStat{q}, res)
	})
	...
	t.Run("a, b, a", func(t *testing.T) {
		...
		query3 := prefixTree.NewQuery("c", 100)
		query4 := prefixTree.NewQuery("d", 100)
			...
			res := &Node{value: "",
				children: []*Node{
					...
					},
					{
						value: "c",
						payload: []*prefixTree.QueryStat{
							prefixTree.NewQuery("c", 100),
						},
					},
					{
						value: "d",
						payload: []*prefixTree.QueryStat{
							prefixTree.NewQuery("d", 100),
						},
					},
				},
				payload: []*prefixTree.QueryStat{
